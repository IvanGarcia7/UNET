...
def evaluate22(net, dataloader, device, amp, zero_samples_ratio=0.9):
    net.eval()
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        ...
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            num_nonzero_samples = len(nonzero_indices)
            num_zero_samples = int(num_nonzero_samples * zero_samples_ratio)
            
            selected_nonzero_indices = torch.randperm(num_nonzero_samples)[:num_nonzero_samples]
            selected_indices = torch.cat([nonzero_indices[selected_nonzero_indices].to(device), torch.randint(high=true_values.numel(), size=(num_zero_samples,)).to(device)])
            
            

	        # Seleccionar solo los valores donde true_values es distinto de 0
            predicted_nonzero = predicted_values.view(-1)[selected_indices]
            true_nonzero = true_values.view(-1)[selected_indices]
...
dir_img = Path('/opt/share/MERIDA/DATASET-TEST/IN/')
dir_mask = Path('/opt/share/MERIDA/DATASET-TEST/OUT/')
dir_checkpoint = Path('/opt/share/MERIDA/Code/Pytorch-UNet/checkpoints/')
...
if __name__ == '__main__':
    ...
    model = UNet(n_channels=8, n_classes=1, bilinear=args.bilinear)
